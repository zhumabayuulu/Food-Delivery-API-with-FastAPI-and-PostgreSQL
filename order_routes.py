...
async def update_order(id: int, order: OrderModel, Authorize: AuthJWT=Depends()):
    ...
    if order_to_update.user != user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="кечиресиз сиз башка адамдардын заказын ондой албайсыз!")
    ...
    custom_response = {
        "success": True,
        "code": 200,
        "message": "сиздин буюртма ийгилукту алмаштырылды",
        "data": {
            "id": order.id,
            "quantity": order.quantity,
...
async def delete_order(id: int, Authorize: AuthJWT=Depends()):
    ...
    if order.user != user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="кечиресиз сиз башка адамдардын буюртмасын очуро албайсыз!")

    if order.order_statuses != "PENDING":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                            detail="кечиресиз сиз жолго чыкан жана жеткезилген буюрманы очуро албайсыз!")
